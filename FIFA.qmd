...
#| fig-alt: "Horizontal grouped bar chart comparing the number of players and team performance (goals plus assists) for top 2022 FIFA World Cup teams. Blue bars represent the number of players, while orange bars represent goals plus assists. France had the highest overall performance, followed by Argentina and Portugal. Most teams had between 20 and 26 players, while performance values ranged from around 10 to 28.."
#| fig-pos: H
#| fig-height: 5
topTeams <- worldCupClean %>%
  slice_max(goals_plus_assists, n = 10) %>%
  select(squad, players, goals_plus_assists) %>%
  pivot_longer(
    cols = c(players, goals_plus_assists),
    names_to = "metric",
    values_to = "value"
  )

ggplot(topTeams,
       aes(x = reorder(squad, value),
           y = value,
           fill = metric)) +
  geom_col(
    position = "dodge",
  ...
  coord_flip() +
  labs(
    title = "Players vs Performance by Team",
    x = "Team",
    y = "Value",
    fill = "Metric"
  ) +
...
The grouped bar chart in @fig-players-vs-performance compares the number of players and goals plus assists for several teams in the 2022 FIFA World Cup. The blue bars show the number of players on each team, and the orange bars show team performance based on goals and assists. France had the highest performance, while Argentina and Portugal also performed well. Some teams, like Brazil and Morocco, had many players but lower goals and assists compared to other teams. This shows that having a larger number of players does not always mean the team will have better offensive performance.
...
#| fig-cap: "Goals + Assists by Team Age Group"
#| fig-alt: "Boxplot comparing goals plus assists for teams with above-average and below-average team age."
#| fig-pos: H
#| fig-height: 5

worldCupClean <- worldCupClean %>%
  mutate(
    age_group = ifelse(
...
ggplot(worldCupClean,
       aes(x = age_group,
           y = goals_plus_assists,
           fill = age_group)) +
  geom_boxplot() +
  labs(
    title = "Goals + Assists by Team Age Group",
    x = "Team Age Group",
    y = "Goals + Assists",
    fill = "Age Group"
  ) +
...
The boxplot in @fig-age-vs-performance compares team performance between teams with above average and below average ages. Teams with below average ages generally show slightly higher offensive performance, which can be seen by the higher median and wider spread of goals and assists. However, there is still overlap between the two groups, which suggests that age alone is not a strong predictor of team success. Overall, age may have some influence on performance, but other factors likely play a larger role.
...
#| fig-alt: "Scatter plot showing the relationship between possession percentage and team performance measured by goals and assists in the 2022 World Cup. Teams with higher possession percentages, such as France, England, and Argentina, generally had better performance."
#| fig-pos: H
#| fig-height: 5
...
This analysis analyzed how team level factors relate to performance in the 2022 FIFA World Cup. The results show that both possession and number of players have moderate positive relationships with goals and assists, suggesting that teams with greater control of the game and more players tend to perform better offensively. In contrast, average team age has a weak negative relationship with performance, indicating that age is not a strong predictor of success. However, these relationships are not strong and include several outliers, meaning that no single variable fully explains team performance. This suggests that success in the World Cup is influenced by a combination of factors beyond those included in this dataset.
...
Chenjian Wei: part of plot and table, help build the qmd file\
Riya Dsouza: Created a graph and table, worked on the README, helped interpret the results and contributed to writing the final report.\
Ryland Jones: Created graphs and tables, made updates to the README, interpreted results
